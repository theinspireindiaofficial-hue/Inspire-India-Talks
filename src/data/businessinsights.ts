...
export default BusinessInsights;import Layout from "@/components/Layout";
import { motion, AnimatePresence } from "framer-motion";
import { useState, useEffect } from "react";
...
const BusinessInsights = () => {
  ...
  // Close the big view with the Escape key
  useEffect(() => {
    ...
    <Layout>
      <section className="relative py-20 overflow-hidden gradient-mesh">
        <div className="container mx-auto px-4 relative z-10">
          ...
          {/* Featured hero — full-bleed image with text overlaid (never feels empty) */}
          {featured && (
            <motion.article
              initial={{ opacity: 0, y: 24 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <Cover article={featured} className="absolute inset-0 w-full h-full" />
              <div className="absolute inset-0 bg-gradient-to-t from-black via-black/60 to-transparent transition-transform duration-700 group-hover:scale-105" />
              <div className="absolute bottom-0 left-0 p-8 md:p-10 max-w-2xl">
                <Meta article={featured} />
                <h2 className="font-serif text-3xl md:text-5xl font-bold mb-4 group-hover:text-primary transition-colors">
                  {featured.title}
