...
const HostEvent = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.success) {
        toast({ title: "Enquiry Received! ✅", description: "Thank you for reaching out. We'll get back to you within 2–3 business days." });
  ...
  const fields = [
    ...
    { name: "organization", label: "Organisation / Institution", type: "text", required: true, placeholder: "Company, college, or NGO name" },
    { name: "audience", label: "Expected Audience Size", type: "text", required: false, placeholder: "e.g. 200–500 students" },
    { name: "date", label: "Preferred Date / Timeframe", type: "text", required: false, placeholder: "e.g. July 2025 or flexible" },
  ];
        ...
        <div className="container mx-auto px-4 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="font-serif text-4xl md:text-6xl font-bold glow-text mb-6 max-w-3xl">
              Bring Inspiration<br /><span className="text-primary">to Your Stage.</span>
            </h1>
            <p className="text-lg md:text-xl text-muted-foreground max-w-2xl">
              Partner with Inspire India Talks to organise a speaker event at your institution, company, or community.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {highlights.map((item, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="glass-card p-8 flex flex-col gap-4">
              <div className="h-12 w-12 rounded-xl bg-primary/10 text-primary flex items-center justify-center ring-1 ring-primary/20">{item.icon}</div>
      ...
      <section className="container mx-auto px-4 py-16 max-w-3xl">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative rounded-3xl overflow-hidden">
          ...
          <div className="relative z-10 p-8 md:p-12">
            ...
            <p className="text-muted-foreground mb-8">Fill in the details below and our team will get in touch within 2–3 business days.</p>
            <form onSubmit={handleSubmit} className="glass-card p-8 rounded-2xl space-y-5">
              ...
              <div>
                <label className="text-xs font-medium uppercase tracking-wider text-muted-foreground mb-2 block">Additional Details</label>
                <textarea name="message" rows={4} placeholder="Tell us more about your event, theme, or any special requirements…"
              ...
              <Button type="submit" className="w-full rounded-xl py-6 text-base font-medium mt-2 hover:shadow-lg hover:shadow-primary/20 transition-all" disabled={isSubmitting}>
                {isSubmitting ? (
